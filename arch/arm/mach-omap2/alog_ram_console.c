...
static int __init alog_ram_console_init(const LogType log_type, AlogRamConsoleBuffer *buffer, size_t buffer_size, char *old_buf)
{
  char reboot_reason[16] = {0};
	...
	// if alog ram console has correct signature, that means there are logs in ram console
        ...
        omap_get_reboot_reason(reboot_reason);            
        printk(KERN_INFO "ram_console: reboot_reason = %s\n",reboot_reason);
        
        if (buffer->sig == alog_ram_console_signature[log_type]  && (!strcmp(reboot_reason,"Panic")||
        	                                     !strcmp(reboot_reason,"system crash")|| 
        	                                     !strcmp(reboot_reason,"exception in sy")||
        	                                     !strcmp(reboot_reason,"wdt"))){        	
...
char omap_last_reboot_reason[OMAP_REBOOT_REASON_SIZE+1] = {0};

int omap_write_reboot_reason(char* reason){
...
void omap_save_reboot_reason(){
	void __iomem *sar_base;

	sar_base = omap4_get_sar_ram_base();
	if(sar_base){
	    strncpy(omap_last_reboot_reason,
	        sar_base + OMAP_REBOOT_REASON_OFFSET, OMAP_REBOOT_REASON_SIZE);
	    //printk("%s, omap_last_reboot_reason = [%s]",__func__,omap_last_reboot_reason);
	}
}

int omap_get_reboot_reason(char* reason){
    if(reason){
 	    strcpy(reason,omap_last_reboot_reason);
 	    //printk("%s, reason = [%s]",__func__,reason);
 	    return strlen(omap_last_reboot_reason);
    }
    return 0;
}
...
#define POWERON_CAUSE_PROC "poweron_cause"

static int alog_ram_console_poweron_cause_show(struct seq_file *s, void *unused){
    seq_printf(s,"%s\n",omap_last_reboot_reason);   
    return 0;
}

static int alog_ram_console_poweron_cause_open(struct inode *inode, struct file *file){
        return single_open(file, alog_ram_console_poweron_cause_show, &inode->i_private);
}

static const struct file_operations alog_ram_console_poweron_cause_fops = {
        .open        = alog_ram_console_poweron_cause_open,
        .read        = seq_read,
        .llseek      = seq_lseek,
        .release     = single_release,
};

static int __init alog_ram_console_poweron_cause_init(void){
    struct proc_dir_entry *entry;
    entry = create_proc_entry(POWERON_CAUSE_PROC, S_IFREG | S_IRUGO, NULL);
    entry->proc_fops = &alog_ram_console_poweron_cause_fops;
    entry->size = OMAP_REBOOT_REASON_SIZE+1;    
    return 0;
}

late_initcall(alog_ram_console_poweron_cause_init);
