...
//Chuck Chen, add for DRAM brand virtaul file, 20120103, Begin
static u32 module_id;
//Chuck Chen, add for DRAM brand virtaul file, 20120103, End
...
static int emif_init(struct omap_hwmod *oh, void *user)
{
	...
	WARN(IS_ERR(od), "Can't build omap_device for %s:%s.\n",
	     name, oh->name);
	//Chuck Chen, add for DRAM brand virtaul file, 20120103, Begin
	printk("%s name=%s, oh->name=%s\n", __FUNCTION__, name,oh->name);
	//Chuck Chen, add for DRAM brand virtaul file, 20120103, Begin
	id++;
	return 0;
...
static int __init omap_init_emif_timings(void)
{
	...
	//Chuck Chen, add for DRAM brand virtaul file, 20120103, Begin
	void __iomem *base = emif[EMIF1].base;
	u32 dram_module_id, dram_scheme, dram_rtl_version;
	dram_scheme = __raw_readl(base + OMAP44XX_EMIF_MOD_ID_REV) & OMAP44XX_REG_SCHEME_MASK;
	dram_module_id = __raw_readl(base + OMAP44XX_EMIF_MOD_ID_REV) & OMAP44XX_REG_MODULE_ID_MASK;
	dram_rtl_version = __raw_readl(base + OMAP44XX_EMIF_MOD_ID_REV) & OMAP44XX_REG_RTL_VERSION_MASK;

	u32 dram_reg_data, dram_reg_cfg_cs;
	dram_reg_data = __raw_readl(base + OMAP44XX_EMIF_LPDDR2_MODE_REG_DATA) & OMAP44XX_REG_VALUE_0_MASK;
	dram_reg_cfg_cs = __raw_readl(base + OMAP44XX_EMIF_LPDDR2_MODE_REG_CFG) & OMAP44XX_REG_CS_MASK;

	module_id = dram_module_id;

	printk("%s DRAM EMIF_MOD_ID_REV, dram_scheme=0x%x dram_module_id=0x%x dram_rtl_version=0x%x\n", __FUNCTION__, dram_scheme, dram_module_id,dram_rtl_version);
	printk("%s DRAM LPDDR2_MODE_REG, dram_reg_data=0x%x , dram_reg_cfg_cs=0x%x", __FUNCTION__,dram_reg_data,dram_reg_cfg_cs);
	//Chuck Chen, add for DRAM brand virtaul file, 20120103, Begin
...
//Chuck Chen, 2012/01/03 add for create DRAM info virtual file, Begin
#include <linux/seq_file.h>
#include <linux/proc_fs.h>

static int dbg_dram_show(struct seq_file *s, void *unused)
{

    if(module_id == 0x440000)
    {
        seq_printf(s,"Elpida\n");
    }
    else
...
static int dbg_dram_open(struct inode *inode, struct file *file)
{
	return single_open(file, dbg_dram_show, &inode->i_private);
}
...
static int __init dram_debuginit(void)
{
    struct proc_dir_entry *entry;
    entry = create_proc_entry("draminfo", S_IFREG | S_IRUGO, NULL);
    entry->proc_fops = &debug_fops;
    entry->size = 32;

	return 0;
}
late_initcall(dram_debuginit);
//Chuck Chen, 2012/01/03 add for create DRAM info virtual file, End
