...
static const u8 twl6040_reg[TWL6040_CACHEREGNUM] = {
	...
	0x40, /* TWL6040_HFLCTL		0x14	*/ /* enable aux out by default */
	0x1D, /* TWL6040_HFLGAIN	0x15	*/
	0x40, /* TWL6040_HFRCTL		0x16	*/ /* enable aux out by default */
...
extern int fih_twl6040_probe(struct snd_soc_codec *codec);
extern int fih_twl6040_remove(struct snd_soc_codec *codec);
 ...
 */
int twl6040_read_reg_volatile(struct snd_soc_codec *codec,
					unsigned int reg)
{
...
/*FihtdcCode@AlanHZChang, 20120202, Detect 3/4 ping headset, begin*/
static int twl6040_hs_jack_adc_raw_read(struct snd_soc_codec *codec)
{
    u8 msb, lsb;
    u8 cachereg, reg;
    int ret = 0;
    ...
    //If codec power is not enable, let it enable.
    if(priv->codec_powered != 1){ 
    ...
    //Enable HBIAS
	cachereg = twl6040_read_reg_cache(codec, TWL6040_REG_AMICBCTL);
	reg = cachereg | 0x1;
	dev_warn(codec->dev, "[1] TWL6040_REG_AMICBCTL = %x\n", reg);
	twl6040_write(codec, TWL6040_REG_AMICBCTL,reg);
	mdelay(100);
        
    ret = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x02, 0x90);
    if (ret) {
            dev_err(codec->dev, "unable to write register 0x90\n");
    }
    ret = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x6f, 0x2e);
    if (ret) {
            dev_err(codec->dev, "unable to write register 0x2e\n");
    }
    ret = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x0e, 0x33);
    if (ret) {
            dev_err(codec->dev, "unable to write register 0x33\n");
    }
        
    mdelay(100);

    ret = twl_i2c_read_u8(TWL6030_MODULE_ID1, &lsb, 0x5b);
    if (ret) {
            dev_err(codec->dev, "unable to read LSB register 0x5b\n");
            return ret;
    }
    ret = twl_i2c_read_u8(TWL6030_MODULE_ID1, &msb,  0x5c);
    if (ret) {
            dev_err(codec->dev, "unable to read MSB register 0x5c\n");
            return ret;
    }

    //Disable HBIAS
	dev_warn(codec->dev, "[2] TWL6040_REG_AMICBCTL = %x\n", cachereg);
	twl6040_write(codec, TWL6040_REG_AMICBCTL,cachereg);
    //disable codec power if not enable
    if(priv->codec_powered != 1){ 
        twl6040_disable(twl6040);
    }

    dev_warn(codec->dev, "LSB status = %x, MSB status = %x\n", lsb, msb);
...
/*FihtdcCode@AlanHZChang, 20120202, Detect 3/4 ping headset, end*/

static void twl6040_hs_jack_report(struct snd_soc_codec *codec,
				   struct snd_soc_jack *jack, int report)
{
	struct twl6040_data *priv = snd_soc_codec_get_drvdata(codec);
	int status, state = BIT_NO_HEADSET/*0*/; 
	...
	if (&priv->hs_jack.sdev){
	    if(fih_get_product_phase() >= Product_PR3){
    		if(!!state){
                int vol;
            	vol = twl6040_hs_jack_adc_raw_read(codec);
            	dev_warn(codec->dev, "voltage=%x\n", vol);
            	if(vol > 0x100) //Headset
            	    state = BIT_HEADSET;
            	else
...
{
    dev_err(codec->dev, "%s, into suspend\n", __FUNCTION__);
...
{	   
    dev_err(codec->dev, "%s, into resume\n", __FUNCTION__);
