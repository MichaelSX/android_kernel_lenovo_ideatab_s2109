...
#ifdef CONFIG_AMPLIFIER_TPA2026D2
static int sdp4430_ext_spk_event(struct snd_soc_dapm_widget *w,
				 struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->dapm->codec;

	if (SND_SOC_DAPM_EVENT_ON(event))
		tpa2026d2_stereo_enable(codec, 1);
	else
		tpa2026d2_stereo_enable(codec, 0);
...
#ifdef CONFIG_AMPLIFIER_TPA2026D2
	SND_SOC_DAPM_SPK("Ext Spk", sdp4430_ext_spk_event),
#else
...
static int sdp4430_twl6040_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (machine_is_omap_4430sdp() || machine_is_omap_tabletblaze() 
		|| machine_is_omap4_panda() || machine_is_omap_4430go7() ||machine_is_omap_4430jem()|| machine_is_omap_4430gox())
...
static int __init sdp4430_soc_init(void)
{
	...
	if (!machine_is_omap_4430sdp() && !machine_is_omap4_panda() && !machine_is_omap_4430go7() && !machine_is_omap_4430jem() && !machine_is_omap_4430gox() &&
