 ...
 * derived from mx50_proximity used on Whitney
 *
...
//#define DRIVER_NAME						"PIC12LF1822_Proximity"
#define DRIVER_NAME						"pic12lf1822_prox"
...
static int pic12lf1822_proximity_read_i2c(u8 *id, u8 reg_num)
{
	...
	error = i2c_smbus_read_byte_data(pic12lf1822_proximity_i2c_client,
		reg_num);
	if (error < 0) {
		return -EIO;
	}
...
static SYSDEV_ATTR(proximity_register, 0644, proximity_register_show,
...
static ssize_t prox_prox_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value_LSB, value_MSB;
	u16 value;

	pic12lf1822_proximity_read_i2c(&value_LSB, PIC12LF1822_PROXL);
	pic12lf1822_proximity_read_i2c(&value_MSB, PIC12LF1822_PROXH);
...
static ssize_t prox_bckgndavg_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value_LSB, value_MSB;
	u16 value;

	pic12lf1822_proximity_read_i2c(&value_LSB, PIC12LF1822_BCKGNDAVGL);
	pic12lf1822_proximity_read_i2c(&value_MSB, PIC12LF1822_BCKGNDAVGH);
	value = (u16)(value_MSB<<8|value_LSB);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_bckgndavg_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u16 value = 0;
	u8 tmp_LSB = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_BCKGNDAVGL, tmp_LSB);
	pic12lf1822_proximity_write_i2c(PIC12LF1822_BCKGNDAVGH, tmp_MSB);
	return size;
}
...
static ssize_t prox_proxstate_store(struct sys_device *dev,
	struct sysdev_attribute *attr, 
					const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXSTATE, value);
	return size;
}
...
static ssize_t prox_proxthres_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{	
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXTHRES);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxthres_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXTHRES, value);
	return size;
}

static SYSDEV_ATTR(proxthres, 0644, prox_proxthres_show, prox_proxthres_store); 

static ssize_t prox_proxnodetect_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXNODETECT);
	return sprintf(buf, "%d\n", value);

}

static ssize_t prox_proxnodetect_store(struct sys_device *dev,
	struct sysdev_attribute *attr, 
					const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXNODETECT, value);
	return size;
}

static SYSDEV_ATTR(proxnodetect, 0644, prox_proxnodetect_show,
	prox_proxnodetect_store); 
...
static ssize_t prox_proxscanrate_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXSCANRATE);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxscanrate_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXSCANRATE, value);
	return size;
}

static SYSDEV_ATTR(proxscanrate, 0644, prox_proxscanrate_show,
	prox_proxscanrate_store); 

static ssize_t prox_proxavgrate_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXAVGRATE);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxavgrate_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXAVGRATE, value);
	return size;
}

static SYSDEV_ATTR(proxavgrate, 0644, prox_proxavgrate_show,
	prox_proxavgrate_store); 

static ssize_t prox_proxtimeout_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value_LSB, value_MSB;
	u16 value;

	pic12lf1822_proximity_read_i2c(&value_LSB, PIC12LF1822_PROXTIMEOUTL);
	pic12lf1822_proximity_read_i2c(&value_MSB, PIC12LF1822_PROXTIMEOUTH);

	value = (u16)(value_MSB<<8|value_LSB);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxtimeout_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u16 value = 0;
	u8 tmp_LSB = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXTIMEOUTL, tmp_LSB);
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXTIMEOUTH, tmp_MSB);
	return size;
}

static SYSDEV_ATTR(proxtimeout, 0644, prox_proxtimeout_show,
	prox_proxtimeout_store); 

static ssize_t prox_proxdbncdet_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXDBNCDET);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxdbncdet_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXDBNCDET, value);
	return size;
}

static SYSDEV_ATTR(proxdbncdet, 0644, prox_proxdbncdet_show,
	prox_proxdbncdet_store); 

static ssize_t prox_proxdbncrel_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXDBNCREL);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxdbncrel_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXDBNCREL, value);
	return size;
}

static SYSDEV_ATTR(proxdbncrel, 0644, prox_proxdbncrel_show,
	prox_proxdbncrel_store); 

static ssize_t prox_proxavgen_show(struct sys_device *dev,
	struct sysdev_attribute *attr, char *buf)
{
	u8 value;

	pic12lf1822_proximity_read_i2c(&value, PIC12LF1822_PROXAVGEN);
	return sprintf(buf, "%d\n", value);
}

static ssize_t prox_proxavgen_store(struct sys_device *dev,
	struct sysdev_attribute *attr, const char *buf, size_t size)
{
	u8 value = 0;
	...
	pic12lf1822_proximity_write_i2c(PIC12LF1822_PROXAVGEN, value);
	return size;
}
	...
	const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "pic12lf1822_proximity_probe\n");
	...
	client->addr = PIC12LF1822_PROXIMITY_I2C_ADDR;
	i2c_set_clientdata(client, info);
	info->client = client;
	pic12lf1822_proximity_i2c_client = info->client;
	pic12lf1822_proximity_i2c_client->addr = PIC12LF1822_PROXIMITY_I2C_ADDR;
...
static irqreturn_t proximity_irq(int irq, void *devid)
{
	...
	/* Debounce for 10 ms */
	schedule_delayed_work(&proximity_work,
		msecs_to_jiffies(PROXIMITY_THRESHOLD));
...
static int __init proximity_port_init(void)
{
	...
	printk(KERN_INFO "Initializing PIC23LF1822 Proximity Sensor\n");
	...
	err = request_irq(irq, proximity_irq, 0, "PIC12LF1822_Proximity", NULL);
	...
	if (err) {
		printk(KERN_ERR "Bowser Proximity: Could not add driver\n");
