	...
	while (!kthread_should_stop()) {
		...
		for (i = 0; int_sts; int_sts >>= 1, i++) {
			...
			if (int_sts & 0x1) {
				...
				if(i!=2)pr_info("twl6030_interrupt_mapping[%d], module_irq(%d) triggered.\n", i, module_irq); 
...
int twl6030_mmc_card_detect_config(void)
{
	...
	fih_twl6030_mmc_card_detect_config(&reg_val);
...
int twl6030_mmc_card_detect(struct device *dev, int slot)
{
	...
	ret = twl_i2c_read_u8(TWL6030_MODULE_ID0, &read_reg,
						TWL6030_MMCCTRL);
	printk("MMC card detect ret: %d, register: %d\n", ret, read_reg);
...
int twl6030_init_irq(int irq_num, unsigned irq_base, unsigned irq_end)
{
	...
	twl6030_irq_next = i;
	pr_info("twl6030-irq: %s (irq %d) chaining IRQs %d..%d\n", "PIH",
