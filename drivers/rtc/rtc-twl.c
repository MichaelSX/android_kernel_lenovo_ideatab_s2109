...
static int twl_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	dev_err(&pdev->dev, "twl_rtc_suspend befor mask rtc_irq_bits=0x%x \n",rtc_irq_bits);
	mask_rtc_irq_bit(BIT_RTC_INTERRUPTS_REG_IT_TIMER_M);
	dev_err(&pdev->dev, "twl_rtc_suspend after mask rtc_irq_bits=0x%x \n",rtc_irq_bits);
...
static int twl_rtc_resume(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "twl_rtc_resume set irqstat=0x%x  \n",irqstat);
	return 0;
}
