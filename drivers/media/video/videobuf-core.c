...
struct videobuf_buffer *videobuf_alloc_vb(struct videobuf_queue *q)
{
	...
	//printk("[HL]%s: +++, q->msize = %d, sizeof(*vb) = %d\n", __func__, q->msize, sizeof(*vb));
	...
	//SW4-L1-HL-Camera-FTM-ReleaseMem-00+_20120208
	//printk("[HL]%s: +++, BEFORE q->int_ops->alloc_vb(q->msize)\n", __func__);
...
static int __videobuf_free(struct videobuf_queue *q)
{
	...
	for (i = 0; i < VIDEO_MAX_FRAME; i++) {
		...
		//SW4-L1-HL-Camera-FTM-ReleaseMem-00+_20120208
		//printk("[HL]%s: A i = %d, NULL != q->bufs[i]\n", __func__, i, i);
			...
			enum v4l2_memory memory)
{
	...
	for (i = 0; i < bcount; i++) {
		...
		//SW4-L1-HL-Camera-FTM-ReleaseMem-00+_20120209
		//printk("[HL]%s: A i = %d, NULL != q->bufs[%d]\n", __func__, i, i);
	...
	//SW4-L1-HL-Camera-FTM-ReleaseMem-00+_20120209
	//printk("[HL]%s: ---, mmap setup: %d buffers, %d bytes each\n", __func__, i, bsize);
		 ...
		 struct v4l2_requestbuffers *req)
{
	...
	//SW4-L1-HL-Camera-FTM-ReleaseMem-00+_20120209
	//printk("[HL]%s: reqbufs: bufs=%d, size=0x%x [%u pages total]\n", __func__, count, size,
	//	(unsigned int)((count * PAGE_ALIGN(size)) >> PAGE_SHIFT));
