...
static inline s32 tiler_stride(u32 tsptr)	//New -- s32 tiler_stride(u32 tsptr)	//Original -- static inline s32 tiler_stride(u32 tsptr)	//SW4-L1-HL-Camera-FTM-ICS-00*_20120111
{
	enum tiler_fmt fmt = TILER_FMT(tsptr);

	//printk("[HL]%s: +++, tsptr = %u, fmt = %d\n", __func__, tsptr, fmt);

	if (fmt == TILFMT_PAGE)
	//{	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
		//printk("[HL]%s: ---, fmt == TILFMT_PAGE, return 0\n", __func__);	
		return 0;
	//}	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
	else if (tsptr & MASK_XY_FLIP)
	//{	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
		//printk("[HL]%s: ---, tsptr & MASK_XY_FLIP, return 1 << (CONT_HEIGHT_BITS + geom[fmt].x_shft)\n", __func__);		
		return 1 << (CONT_HEIGHT_BITS + geom[fmt].x_shft);
	//}	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
	else
	//{	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
		//printk("[HL]%s: ---, return 1 << (CONT_WIDTH_BITS + geom[fmt].y_shft)\n", __func__);		
		return 1 << (CONT_WIDTH_BITS + geom[fmt].y_shft);
	//}	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
}
//EXPORT_SYMBOL(tiler_stride);	//SW4-L1-HL-Camera-FTM-ICS-00-_20120111
...
u32 tiler_pstride(const struct tiler_block_t *b)
{
	...
	if (fmt == TILFMT_PAGE)
	//{	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111
		//printk("[HL]%s: ---, fmt == TILFMT_PAGE, return tiler_vstride(b)\n", __func__);
		return tiler_vstride(b);
	//}	//SW4-L1-HL-Camera-FTM-ICS-00+_20120111

	//printk("[HL]%s: ---, return tiler_stride(b->phys & ~MASK_VIEW)\n", __func__);
