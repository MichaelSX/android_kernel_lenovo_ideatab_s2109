...
static void tc358765_disable(struct omap_dss_device *dssdev)
{
	...
	if (hpd == 0){	
	gpio_request(60,"HDMI_60");
	gpio_direction_output(60, 1);
	gpio_set_value(60,0);
	//pr_err("gpio_60=%d\n",gpio_get_value(60));
	//printk("gpio_60=%d\n",gpio_get_value(60));
	gpio_request(41,"HDMI_41");
...
static int tc358765_enable(struct omap_dss_device *dssdev)
{
	struct tc358765_data *d2d = dev_get_drvdata(&dssdev->dev);
	int r = 0;
	printk(" enable LCD start \n");
	dev_dbg(&dssdev->dev, "enable\n");

	if (dssdev->state != OMAP_DSS_DISPLAY_DISABLED)
		return -EINVAL;

	if (gox_lcm_power_on_check())
	{
		gox_lcm_power_off();  
		if(cmdline_query("androidboot.bootmode=recovery")||cmdline_query("androidboot.bootmode=ftm"))
		{
			printk("LCD Power set to LOW and then slepp 100ms\n");	
			msleep(100);
		}
		else
			printk("LCD Power set to LOW\n");	
	}
...
static int tc358765_suspend(struct omap_dss_device *dssdev)
{
	...
	printk(" suspend LCD start \n");
	tc358765_disable(dssdev);
	printk(" suspend LCD end \n");
	return 0;
}
