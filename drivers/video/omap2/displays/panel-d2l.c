...
int d2l_write_register(u16 reg, u32 value)
{
	...
	//udelay(100);//TODO:check with TI for delay timing
	mdelay(100);
...
static ssize_t d2l_show_status(struct device *dev,
		struct device_attribute *attr, char *buf)
{

	struct omap_dss_device *dssdev = to_dss_device(dev);
	struct d2l_data *d2d = dev_get_drvdata(&dssdev->dev);
	ulong status0 = 0, status1 = 0, status = 0, sys_stat = 0;
        int ret = 0;
	...
	if (d2d->enabled) {
                status0 = d2l_i2c_read(DSI_LANESTATUS0);
                status1 = d2l_i2c_read(DSI_LANESTATUS1);
						
                status = d2l_i2c_read(DSI_INTSTATUS);
                sys_stat = d2l_i2c_read(SYSSTAT);							
	} else {
                dev_err(dev, "d2d is not enabled.\n");
                ret = -ENODEV;
	}
	...
	return  snprintf(buf, PAGE_SIZE, "DSI_LANESTATUS0=0x%lu\nDSI_LANESTATUS1=0x%lu\nDSI_INTSTATUS=0x%lu\nSYSSTAT=0x%lu\n", 
                         status0,status1,status,sys_stat);
}

static DEVICE_ATTR(d2l_status, S_IRUGO, d2l_show_status, NULL);
...
void d2l_config(void)
{
	...
	//d2l_write_register(VPCTRL, 0x00000120); /* vtgen on */
	d2l_write_register(VPCTRL, 0x00000020); /* vtgen on */
	//d2l_write_register(VPCTRL, 0x00F00010); /* vtgen on */

	d2l_write_register(HTIM1, (D2L_HBP << 16) | D2L_HSW);
	d2l_write_register(HTIM2, (D2L_HFP << 16) | D2L_WIDTH);
	d2l_write_register(VTIM1, (D2L_VBP << 16) | D2L_VSW);
	d2l_write_register(VTIM2, (D2L_VFP << 16) | D2L_HEIGHT);
	...
	/* clean all flag status for DSI_INTSTATUS check*/
	d2l_write_register(DSI_INTCLR, 0xFFFFFFFF);	
}
