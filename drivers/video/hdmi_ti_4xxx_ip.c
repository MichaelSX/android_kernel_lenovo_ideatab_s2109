...
static int hdmi_set_phy_pwr(struct hdmi_ip_data *ip_data,
				enum hdmi_phy_pwr val, enum hdmi_phy_pwr_change_reason reason)
{
	/* FIXME audio driver should have already stopped, but not yet */
	bool wait_for_audio_stop = (reason != RESYNC) &&(reason != MODE_CHANGE);
...
int hdmi_ti_4xxx_phy_init(struct hdmi_ip_data *ip_data)
{
	...
	r = hdmi_set_phy_pwr(ip_data, HDMI_PHYPWRCMD_LDOON, NORMAL);
	if (r)
		return r;

	r = hdmi_set_phy_pwr(ip_data, HDMI_PHYPWRCMD_TXON, NORMAL);
	if (r)
		return r;
...
void hdmi_ti_4xxx_phy_off(struct hdmi_ip_data *ip_data, enum hdmi_phy_pwr_change_reason reason)
{
