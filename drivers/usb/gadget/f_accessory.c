...
static struct miscdevice acc_device_switch_adb = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "android_scsi_adb",
	.fops = &acc_fops,
};
...
void android_for_scsi_adb_open(int adb_open_flag)
{
	char *adb_connected[2] = { "ADB_ENABLE=enable", NULL };
	char *adb_disconnected[2] = { "ADB_ENABLE=disable", NULL };
	char **uevent_envp = NULL;
	if(adb_open_flag==1)
		uevent_envp=adb_connected;
	else
		uevent_envp=adb_disconnected;
	kobject_uevent_env(&acc_device_switch_adb.this_device->kobj, KOBJ_CHANGE, uevent_envp);
	printk(KERN_INFO "%s: sent uevent %s\n", __func__, uevent_envp[0]);
}
EXPORT_SYMBOL(android_for_scsi_adb_open);
...
static int acc_setup(void)
{
	...
	ret = misc_register(&acc_device_switch_adb);
	if (ret)
		goto err;
