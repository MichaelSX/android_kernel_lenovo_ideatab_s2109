...
static void musb_otg_notifier_work(struct work_struct *data_notifier_work)
{
	...
	case USB_EVENT_ID:
		...
		if (is_otg_enabled(musb)) {
			...
			if (musb->gadget_driver) {
				...
				//20120309@WinCYPeng add for TI Patch Subject: [PATCH] usb: musb: Turn off the PHY while doing interface selection
	...
	case USB_EVENT_NONE:
		...
		if (!is_autosuspend && !pm_runtime_suspended(musb->controller))
